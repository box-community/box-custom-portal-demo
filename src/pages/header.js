...
const Header = tw.header`
  ...
  max-w-screen-xl mx-auto shadow
`;
...
export const NavLink = tw.a`
  text-lg my-2 lg:text-lg lg:mx-6 lg:my-0 no-underline	
...
`;
export const  active = tw`border-primary-500!`;
...
const page = location.pathname.substring(location.pathname.lastIndexOf('/')+1);
  ...
  const defaultLogoLink = (
    [<LogoLink key="logo" href='/index'>Increo Financial</LogoLink>]
  );
  ...
  return (
    <Header className={className || "header-light"}>
      <DesktopNavLinks key="12" css={collapseBreakpointCss.desktopNavLinks}>
        {logoLink}
        <NavLinks key="1">

       {navigation.map(nav=>
        ( <NavLink  key={nav.page} href={nav.page}>{page===nav.page?'*':''} {nav.title}</NavLink>))
        }
          </NavLinks>
    <NavLinks key="2">
      <NavLink key="logout" href={loginStateURL} tw="lg:ml-12!">
        {loginStateLabel}
      </NavLink>
